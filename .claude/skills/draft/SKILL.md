...
5. **Cumulative integrity (revision-specific) — both directions.** When revising, check both losses AND retentions:
   - **(a) Lost content.** Has any earlier *necessary* content been silently dropped by the patch? A correction to one phrase shouldn't cut the apology, warmth marker, or substantive engagement that was correctly in the prior version.
   - **(b) Retained stale connective tissue.** Has any earlier bridging word been *retained* that no longer fits the new structure? Words like `"Otherwise"`, `"However"`, `"Also"`, `"In addition"`, `"Still"`, `"In that case"` bridge between two specific things — when the patch changes one of the two things, the bridge can become stale or wrong. Example: prior version had `"I'm around anytime today. Otherwise, when works for you?"` (where `Otherwise` = "if today doesn't work"). Patching the second sentence to `"You can give me a window..."` left `"Otherwise"` forcing an artificial either/or (anytime-today XOR window) when in fact a window can be for today. Re-read the message as a fresh whole, not as a patched version.
6. **Recipient POV — joint implications, read linearly.** Each clause can be locally true and grounded while the joint message implies something about the recipient they haven't signaled. Read the draft linearly as the recipient would, with their mental state, and ask what the message asks them to assume or do. Clause-by-clause grounding is necessary but not sufficient. Two failure modes:
   - **(a) Clauses combine to imply a third claim about the recipient.** *"I'm around anytime today. What time works for you?"* — both clauses derive from Gil's stated availability (true), but the joint frame asks the recipient to pick a time today when they've signaled nothing about today (false implication).
   - **(b) An earlier clause anchors a frame that later clauses read inside.** *"I'm around anytime today. Send me a window."* — sentence 2 is generic in isolation, but in linear reading sentence 1 sets "today" as the frame and sentence 2 is read as "send me a window today." Clause-by-clause grounding misses this; only reading-in-order with the recipient's mental state catches it.

Fix: ask them to signal first (open ask leads), reframe binding statements as explicitly conditional (`"if it works for you"`), or remove the frame-anchor clause entirely.
