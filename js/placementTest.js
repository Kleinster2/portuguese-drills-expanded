...
function startPhase(phaseNum) {
  ...
  const introHTML = `
    <div class="flex items-start space-x-3 mb-4">
      <div class="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 font-bold">${phaseNum}</div>
...
function renderProductionQuestion(question, index) {
  ...
  const scenarioHTML = question.scenario ? 
    `<div class="mb-3 p-3 bg-amber-50 border-l-4 border-amber-400 rounded text-sm text-amber-900 italic">${question.scenario}</div>` : '';
  ...
  const chipsHTML = shuffledChips.map((chip, idx) => `
    <button onclick="selectDiagnosticChip(${question.id}, '${chip.replace(/'/g, "\'")}')" 
            ...
            class="px-3 py-2 bg-white border border-slate-300 rounded hover:bg-blue-50 hover:border-blue-500 transition text-sm">
      ${chip}
    </button>
  `).join('');
  ...
  const html = `
    <div class="flex items-start space-x-3 mb-6" id="q-container-${question.id}">
      ...
      <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm w-full max-w-2xl">
        ...
        <div class="text-xl font-mono text-slate-700 mb-6 bg-slate-50 p-4 rounded-lg border border-slate-100">
          ${templateHTML}
        </div>
        ...
        <div class="flex justify-between items-center mt-4 pt-4 border-t border-slate-100">
           <button onclick="skipDiagnosticQuestion(${question.id})" class="text-slate-400 hover:text-slate-600 text-sm font-medium">I don't know</button>
...
window.selectDiagnosticChip = function(qid, chip) {
  ...
  Array.from(container.children).forEach(btn => {
    if (btn.textContent.trim() === chip) {
      btn.classList.add('bg-blue-100', 'border-blue-500', 'ring-2', 'ring-blue-200');
    } else {
...
function showPhaseSuccess(nextPhase) {
  ...
  container.insertAdjacentHTML('beforeend', `
    <div class="my-4 p-4 bg-green-100 text-green-800 rounded-xl text-center font-bold animate-pulse">
...
function finishTest() {
  ...
  const html = `
    <div class="mt-8 bg-slate-900 text-white p-8 rounded-3xl shadow-2xl">
      ...
      <div class="bg-white text-slate-900 p-4 rounded-xl mb-4">
